...
# Density-only model-based reconstruction with KomaMRI and finite differences
...
########################################################################
# Local input files from the KomaMRI sensitivity-map how-to
########################################################################
...
########################################################################
# Acquisition and coil sensitivities
########################################################################
...
image_profiles = length(acquired_lines)

raw_reference = RawAcquisitionData(ISMRMRDFile(fully_sampled_mrd_file))
raw_reference.profiles = raw_reference.profiles[4:131]
...
measured_data = reduce(
    vcat,
    profile.data for profile in raw_measured.profiles[4:(3 + image_profiles)]
)

# Only simulate the navigators and first accelerated image average.
adc_blocks = findall(block -> is_ADC_on(seq[block]), 1:length(seq))
seq = seq[1:adc_blocks[navigators + image_profiles]]
...
spins_per_density = "--quick" in ARGS ? 10 : 50_000
...
basis_signals = map(density_levels) do density
    ...
    count = min(spins_per_density, length(indices))
    ...
    reduce(vcat, profile.data for profile in raw.profiles[4:end])
end
...
function gradient!(gradient, parameters)
...
println("Original density levels:  ", density_levels)
